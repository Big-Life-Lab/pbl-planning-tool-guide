...
(caseExamples <- data.frame(
    Name = c("Health status report", "Canada bikes like the Dutch", "Healthy cities", "Adjusting gears"),
    Type = c("Filter, Stratification, Scenario - Health behaviour attributable", "Filter, Scenario - Intervention: Absolute", "Filter, Stratification, Scenario - Intervention: Target", "Filter, Scenarion - Intervention: Absolute"),
    Description = c("Generate statistic for a health status report", "Impact on life expectancy by increasing cycling rates", "Impact on life expectancy by modifying all health behaviours", "Impact on life expectancy from increasing cycling infrastructure")) 
    )#closingbracket 
```
```{r, echo=FALSE}
#Creating the table
...
6. Select the health behaviours: physical activity and diet

7. Title the calculation: Alberta: Physical activity and diet attributable life expectancy
...
C. What is the average difference in daily cycling transporation for Canadians and the Dutch?
...
<div class="FOO">
**Note:** Since the physical activity intervention scenario on the Project Big Life Planning Tool is for daily leisure energery expenditure and not daily active transportation, we will use the 'absolute' scenario for this example. We will increase each individual's daily liesure energy expenditure by 1.28 MET-hours (1.35 MET-hours - 0.07 MET-hours).
</div>
...
- **Diet**: Calculate the average daily consumption of each diet componet (variables: FVCDCAR, FVCDFRU, FVCDJUI, FVCDPOT, FVCDSAL, and FVCDVEG) in the XXX and Ottawa region. 
...
You want to determine the impact of the 100km of new cycling infrastructure on Hamilton’s life expectancy.
...
1. Convert the increased duration of cycling into daily energy expenditure (MET-hours): $\text{MET-hours} = \text{Duration}*\text{MET value for cycling}$ 
...
8. Name your calculation: Adjusting gears - Hamilton life expectancy
