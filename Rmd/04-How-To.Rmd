...
The planning tool requires the CCHS 2013/14 PUMF or Shared version as a csv format. Other population health surveys can be used if the required variables are transformed to the specificationso fo the CCHS 2013/14. There are planx to support the CCHS versions from 2001 to 2013. 

Prior to using the Project Big Life Planning Tool you may want to transform your data set. Reasons include: customized filter(s) and/or customized stratification(s). Any variable that you create can be selected within the planning tool in the filter and/or stratify options.
...
Only one data set can be used on the platform for each calculation; a calculation cannot be preformed across multiple data sets. However, prior to using the planning tool, you can merge multiple datasets to the CCHS 2013/14 format.
