...
- **sample.csv (Sample)** this is fabricated data set that includes all variables required for calculation, recommend for calculation, and needed for the [case examples](#caseexamples). This data set is based on the 2013/2014 CCHS PUMF but it is **entirely fabricated and can only be used for examplary purposes.**
- **sample_small.csv (Sample)** is the same data set as above but has fewer observations. Use this data set if you are testing out the platform as it will take less time to compute. 


**Steps**

1. Click on the checkmark beside the "Sample file" to select it. 
...
3. Choose the file type (2013/2014 PUMF or 2013/2014 Shared) for your uploaded file, in the table that showcases all of the files.
...
### Attributable fraction scenarios

Atttributable fraction scenarios provide you with the ability to see the best case scenario for the population. These scenarios replicate the commonly used method by Levin{ref}. For example:
...
2. Check the button beside: "Health behaviour attribution" under the "Scenario" drop down.

3. Check the box beside the health behaviour that you want to have a health behaviour attribution calculation.
...
3. A new table will appear under the calculate button. The table includes the status of the calculation, the calculation: name, measures, filters and stratifications that were select. 
...
1. Click on the calculation name, in the table under the green calculation button. 

2. There are three tabs for the results:

- **Overview:** provides a summary of the calculation that was preformed including: measure, filters used, errors, and warnings.

- **Stratifications:** This will show the results for the measurement you choose to calculate. This screen will either have:
    - **a table** when you stratifications are selected.
    - **a graph** when stratification are selected. The graph is further described below.

- **Log:** will give you a summary of the warnings and errors that occurred during the calculation.
...
Scroll down in the pop up window and select "Download File: Your_file_and_calculation_name".

Two files will be downloaded. 

- **Text file:** will give you a summary of the calculation preformed.
- **CSV file:** will provide you a table with the measures you preformed.
