...
Risk of the health outcome (e.g. risk of dying) is the outcome of the multivariable predictive risk algorithm. Mutlivariable risk algorithms may be separately created for strata. MPoRT (internal link), for example, has one algorithm for males, and one algorithm for females. An example of the mutlivariable risk algorithm is MPoRT, a cox-proportional hazard model:
...
1. Risk of the health outcome is calculated for each respondent (row) in the data set using the mutlivariable predictive risk algorithm.
2. If the data has a study weight, the each row calculation is weighted by multiplying the health outcome by the survey weight. For example, all CCHS surveys are have weigh variables (WTS_M for the Public Micro Use File or WTS_S for the shared file).
3. The health outcome weighted mean is calculated.
...
(Maybe modify this formulat and put it in the number of health outcomes section?)
...
1. The probability of death for a person's current age is calculated using the respective MPoRT (male or female), and the individual's health profile (e.g. never smoked, 15 drinks weekly, hypertension, Canadian Citizen, etc) (e.g. $q_x$, where $x=43$). 

2. The probability of death is recalculated for additional rows (incremental older ages) up to age 90 years ( $q_{x+1}, q_x{x+2}, ..., q_90}$). For each life table row, age is the only variable that is changed for MPoRT risk calculation. 
